...
async function searchRobloxPlayer(query) {
  ...
  if (!isNaN(query)) {
    ...
    const searchUrl = `https://users.roblox.com/v1/users/search?keyword=${encodeURIComponent(
      query
    )}&limit=10`
...
async function handleRobloxServersView(interaction, cache, page = 0) {
  ...
  if (!publicServers || publicServers.length === 0) {
    return interaction.reply({
      content: "❌ No hay servidores públicos disponibles.",
      ephemeral: true,
    })
  }
  ...
  let serversList = `**🌐 SERVIDORES PÚBLICOS (Página ${
    page + 1
  }/${totalPages}):**\n\n`
  ...
  currentServers.forEach((server, index) => {
    ...
    serversList += `⏱️ Ping: ${server.ping || "N/A"}ms\n`
    serversList += `🗺️ Región: ${server.location || "Global"}\n`
...
async function handleGamePassesView(interaction, cache, page = 0) {
  ...
  try {
    ...
    let passesList = `**🎮 PASES DEL JUEGO (Página ${
      page + 1
    }/${totalPages}):**\n\n`
...
async function handleRobloxNavigation(interaction, action) {
  ...
  if (!cache) {
    ...
    return interaction.reply({
      content:
        "❌ No hay datos de juego disponibles. Usa `.roblox [juego]` primero.",
      ephemeral: true,
    })
  }
  ...
  try {
    if (action === "searchPlayerRoblox") {
      ...
      await handleGamePassesView(
        interaction,
        cache,
        Math.max(0, cache.passPage - 1)
      )
      ...
      await handleRobloxServersView(
        interaction,
        cache,
        Math.max(0, cache.serversPage - 1)
      )
      ...
      await interaction.deferUpdate() // Defer the update for refresh action
      try {
        ...
        if (!gameData) {
          return interaction.editReply({
            content: "❌ Error al actualizar datos del servidor.",
          })
        }
        ...
        const totalPlayers = publicServers.reduce(
          (sum, server) => sum + server.playing,
          0
        )
        const totalMaxPlayers = publicServers.reduce(
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `**📊 Estadísticas del Juego:**\n\n` +
              ...
              `👥 Jugadores: ${totalPlayers.toLocaleString()}\n\n` +
              `**ℹ️ Información General:**\n` +
              `📈 Rating: ${gameData.totalUpVotes?.toLocaleString() || 0}👍 / ${
                gameData.totalDownVotes?.toLocaleString() || 0
              }👎\n` +
              `👁️ Visitas: ${gameData.visits?.toLocaleString() || "N/A"}\n` +
              `🎮 Jugando ahora: ${
                gameData.playing?.toLocaleString() || totalPlayers.toLocaleString()
              }`
          )
          .setColor("#00b2ff")
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `**📊 Estadísticas del Juego:**\n\n` +
            `**👥 JUGADORES TOTALES:** ${cache.totalPlayers.toLocaleString()}/${
              cache.totalMaxPlayers?.toLocaleString() || "N/A"
            }**\n\n` +
            `**🌐 Servidores Públicos:**\n` +
            `📊 Servidores: ${cache.totalServers}\n` +
            `👥 Jugadores: ${cache.totalPlayers.toLocaleString()}\n\n` +
            `**ℹ️ Información General:**\n` +
            `📈 Rating: ${cache.gameData.totalUpVotes?.toLocaleString() || 0}👍 / ${
              cache.gameData.totalDownVotes?.toLocaleString() || 0
            }👎\n` +
            `👁️ Visitas: ${cache.gameData.visits?.toLocaleString() || "N/A"}\n` +
    ...
    return interaction.reply({
      content: "❌ Error procesando la acción. Intenta de nuevo.",
      ephemeral: true,
    })
  }
}

// Define MONITORED_GAMES_IDS globally or pass it to the function
...
async function handleRobloxSearch(message, args) {
  // If no arguments are provided, display images of monitored games
  if (args.length === 0) {
    try {
      const embeds = [];
      for (const universeId of MONITORED_GAMES_IDS) {
        ...
        if (gameData) {
          const embed = new EmbedBuilder()
            ...
            .setImage(gameIcon) // Set the image of the game
            .setColor("#00b2ff")
            .setFooter({ text: `ID: ${gameData.placeId} | Universe ID: ${universeId}` })
            .setTimestamp();
          embeds.push(embed);
        }
      }
      if (embeds.length > 0) {
        ...
        await message.reply("❌ No se pudieron obtener imágenes para los juegos monitoreados.");
      }
    } catch (error) {
      console.error("Error al obtener imágenes de juegos monitoreados:", error.message);
      await message.reply("❌ Error al obtener las imágenes de los juegos monitoreados.");
    }
    return; // Exit the function after displaying images
  }

  // Original logic for searching a specific game (if args are provided)
  const input = args.join(" ")
  if (!input)
    return message.reply(
      "❗ Debes escribir el ID del juego de Roblox o el nombre."
    )
  ...
  try {
    ...
    if (!isNaN(input)) {
      ...
      try {
        ...
        return message.reply(
          "❌ No se pudo encontrar el juego con ese ID."
        )
      }
    } else {
      const searchUrl = `https://games.roblox.com/v1/games/list?model.keyword=${encodeURIComponent(
        input
      ...
      if (!games.length) {
        const broadSearchUrl = `https://catalog.roblox.com/v1/search/items?category=Experiences&keyword=${encodeURIComponent(
          input
        )}&limit=10`
        try {
    ...
    return message.reply(
      `❌ Error al obtener la información de Roblox: ${error.message}`
    )
  }
}
